<?php
	...
	if (isset($_POST['btnRegPet']))
	{
		...
		$age = $_POST['age'];
		$gender = $_POST['gender'];
		$vetpet = $_POST['inCharge'];
		$image = $_FILES['image']['name'];
		$target = "../assets/img/".basename($_FILES['image']['name']);

		$query = "INSERT INTO pets_owner(avatar,pet_name,pet_type,pet_owner,address,breed,age,gender,veterinarian)VALUES('$image','$pname','$ptype','$oname','$oadd','$pbreed','$age','$gender','$vetpet')";
		$result = mysqli_query($connection,$query)or die(mysqli_error($connection));

		if ($result and move_uploaded_file($_FILES['image']['tmp_name'], $target))
		{
			$select = "SELECT * FROM pets_owner WHERE pet_name = '$pname'";
			...
			if ($num > 0)
			{
				...
				<script type="text/javascript">
					function back()
					{
						window.location.href='petowner_record.php';
					}
				</script>
				<?php
			}
